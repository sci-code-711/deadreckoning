...
for r in range(1,numRows):
    ...
    dev_bod=(np.absolute(((P.loc[r,"ax"])**2+(P.loc[r,"ay"])**2+(P.loc[r,"az"])**2)-gsize**2))**0.5
    ...
    P.loc[r,"X"]=P.loc[r-1,"X"]+((P.loc[r-1,"VX"])*dt)+((P.loc[r-1,"ax"]+P.loc[r,"ax"])*(dt**2)/4) 
    P.loc[r,"Y"]=P.loc[r-1,"Y"]+((P.loc[r-1,"VY"])*dt)+((P.loc[r-1,"ax"]+P.loc[r,"ax"])*(dt**2)/4) 
    P.loc[r,"Z"]=P.loc[r-1,"Z"]+((P.loc[r-1,"VZ"])*dt)+((P.loc[r-1,"az"]+P.loc[r,"az"])*(dt**2)/4)
