...
P.columns=["t","ax","ay","az","vl","vm","vn","VX","VY","VZ","L","M","N","X","Y","Z","qw","qx","qy","qz"] # adds indexes to data frame
...
qc=Q.quaternion(q[0]/(norm),q[1]/(norm),q[2]/(norm),q[3]/(norm))
...
a=Q.quaternion(0,P.loc[0,"ax"],P.loc[0,"ay"],P.loc[0,"az"]) # converts initail accelerometer reading into a quaternian
...
for r in range(1,numRows):
    ...
    for row in range(rmin,r):
        acel_pred[fut_r-(r-row),1]=(P.loc[row,"ax"]**2+P.loc[row,"ay"]**2+P.loc[row,"az"]**2)**0.5
    ...
    for row in range(r,rmax):
        ...
        aq_fut=Q.quaternion(0,data.loc[row,"ax"],data.loc[row,"ay"],data.loc[row,"az"])
    ...
    if dev_max<0.035:
        ...
        qhead=Q.quaternion(qc.w/np.sqrt(qc.w**2+qc.z**2),0,0,qc.z/np.sqrt(qc.w**2+qc.z**2))
        ...
        for row in range(rmin,rmax):
            ...
            if np.linalg.norm(np.cross(ag,g))==0:
                ...
                qg=qhead*Q.quaternion(qgw,qgx,qgy,qgz)
        ...
        aq=Q.quaternion(0,data.loc[r,"ax"],data.loc[r,"ay"],data.loc[r,"az"])
    ...
    P.loc[r,"VX"]=round(P.loc[r-1,"VX"]+((P.loc[r-1,"ax"]+P.loc[r,"ax"])*dt/2),10)
    P.loc[r,"VY"]=round(P.loc[r-1,"VY"]+((P.loc[r-1,"ay"]+P.loc[r,"ay"])*dt/2),10)
    P.loc[r,"VZ"]=round(P.loc[r-1,"VZ"]+((P.loc[r-1,"az"]+P.loc[r,"az"])*dt/2),10)
    
    # intergrates linea velocities to find linear displacments
    P.loc[r,"X"]=round(P.loc[r-1,"X"]+((P.loc[r-1,"VX"])*dt)+((P.loc[r-1,"ax"]+P.loc[r,"ax"])*(dt**2)/4),10) 
    P.loc[r,"Y"]=round(P.loc[r-1,"Y"]+((P.loc[r-1,"VY"])*dt)+((P.loc[r-1,"ax"]+P.loc[r,"ax"])*(dt**2)/4),10) 
    P.loc[r,"Z"]=round(P.loc[r-1,"Z"]+((P.loc[r-1,"VZ"])*dt)+((P.loc[r-1,"az"]+P.loc[r,"az"])*(dt**2)/4),10)
    
print("gcal_count:")
print(count)
P.to_csv('Temp_EKF_fut_Positional.csv', index=False) # saves data table of reconstructed trajectory into a csv file
